...
describe('Product API', () => {
    ...
    it('should get a product by ID', async () => {
        const response = await request(app).get(`/products/${seedProducts[0].productId}`);
        expect(response.status).toBe(200);
    ...
    it('should update a product by ID', async () => {
        ...
        const response = await request(app).put(`/products/${seedProducts[0].productId}`).send(updatedProduct);
        expect(response.status).toBe(200);
        expect(response.body).toEqual(updatedProduct);
    });

    it('should delete a product by ID', async () => {
        const response = await request(app).delete(`/products/${seedProducts[0].productId}`);
